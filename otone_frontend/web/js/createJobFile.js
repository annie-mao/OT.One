...
function makePipettingMotion (theDeck, theTool, thisParams, shouldDropPlunger) {
  ...
  if(theDeck[containerName] && theDeck[containerName].locations) {
    ...
    if(thisParams['liquid-tracking']===true) {
      // ** ANNIE'S EDITS **
      // add a negative heightChangeOffset for aspiration (to keep pipette tip below liquid level) and
      // a positive offset for dispense (to keep pipette tip above liquid level), if using liquid tracking
      // with an added 2mm below liquid level or 2mm above liquid level offset
      //var heightChangeOffset=(thisParams['volume']/locationPos['total-liquid-volume'])*locationPos['depth']*Math.sign(thisParams['volume']);
      arriveDepth = specifiedOffset-locationPos['current-liquid-offset']-2;
    }
    else{
    ...
    // ** ANNIE'S EDITS ** stay below liquid level for small volumes
    if(Math.abs(thisParams['volume'])>10){
    // go to the top of the well
    // ** ANNIE'S EDITS ** 
    // go to the current liquid offset if using liquid tracking
      if(thisParams['liquid-tracking']){
        moveArray.push({
          'z' : specifiedOffset - locationPos['current-liquid-offset'],
          'container' : containerName
        });
      }
      else{
        moveArray.push({
          'z' : -1, // -1 to ensure tip touch will be slightly below container top
          'container' : containerName
        });
      }
    }
    ...
    // **ANNIE'S EDITS** touch tip 1 below liquid level for lower volumes
    //  moveArray.push({
	//'z' : 1,
	//'relative' : true
      //});
